...
int main() {
    ...
    vector<size_t> circDistV;
    for (auto&& i : circuitSet) {
        circDistV.push_back(i->size());
    }
    sort(circDistV.begin(), circDistV.end());

    cout << circDistV[circDistV.size() - 1] * circDistV[circDistV.size() - 2] * circDistV[circDistV.size() - 3] << endl;
}